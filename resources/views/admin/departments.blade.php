            ...
            <div class="page-header d-flex align-items-center justify-content-between flex-wrap"
                ...
                <div style="flex: 1; min-width: 0; padding-right: 1rem;">
                    <h5 class="mb-1" style="font-family: 'Fraunces', Georgia, serif; color: var(--navy);"><i
                            class="bi bi-building me-2"></i>Departments</h5>
                    <p class="text-muted small mb-0">Departments serve as organizational units that can be assigned to
                        administrators, enabling them to manage associated facilities, equipment, and extra services.</p>
                </div>
    ...
    <script>
        ...
        async function loadDepartments() {
            ...
            try {
                ...
                if (response.ok && result.success !== false) {
                    ...
                    throw new Error(result.message || `HTTP ${response.status}: Failed to load departments`);
                }
            } catch (error) {
        ...
        function renderDepartments() {
            ...
            departmentsData.forEach(dept => {
                ...
                const adminListHtml = adminCount > 0 ?
                    dept.admins.map(admin => `
                                <div class="admin-badge">
                                    <i class="bi bi-person-circle me-1"></i>
                                    ${admin.first_name} ${admin.last_name}
                                    ${admin.pivot?.role_name ? `<span class="badge bg-primary ms-1" style="font-size: 0.65rem;">${admin.pivot.role_name}</span>` : ''}
                                </div>
                            `).join('') :
                    '<p class="text-muted mb-0">No administrators assigned</p>';

                const card = `
                    <div class="card dept-card border shadow-sm mb-3 p-0" style="border-radius: var(--radius-md);">
                        <div class="card-header w-100 d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, var(--navy) 0%, var(--navy-mid) 100%); color: var(--white); padding: 0.75rem 1.25rem; border-radius: var(--radius-md) var(--radius-md) 0 0;">
                            ...
                            <div class="d-flex align-items-center gap-1">
                                <button class="btn btn-sm btn-light d-inline-flex align-items-center justify-content-center px-2 py-1" style="min-width: 32px;" onclick="editDepartment(${dept.department_id})">
                                    <i class="bi bi-pencil" style="color: var(--navy);"></i>
                                </button>
                                <button class="btn btn-sm btn-light d-inline-flex align-items-center justify-content-center px-2 py-1" style="min-width: 32px;" onclick="deleteDepartment(${dept.department_id})">
            ...
            document.querySelectorAll('.toggle-admins-btn').forEach(btn => {
                btn.addEventListener('click', function () {
                    const deptId = this.dataset.deptId;
                    const container = document.getElementById(`adminList_${deptId}`);
                    const icon = this.querySelector('i');

                    if (container.style.display === 'none') {
                        container.style.display = 'block';
                        icon.className = 'bi bi-chevron-up';
                        this.innerHTML = '<i class="bi bi-chevron-up"></i> Hide Admins';
                    } else {
                        container.style.display = 'none';
                        icon.className = 'bi bi-chevron-down';
                        this.innerHTML = '<i class="bi bi-chevron-down"></i> Show Admins';
                    }
                });
            });
        }
