...
<main id="main">
    <div class="container-fluid px-4">
        ...
        <div class="page-header">
            <h5><i class="bi bi-tag me-2"></i>Request Purposes</h5>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addPurposeForm">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Route To Administrator</label>
                        <select class="form-select" name="routed_admin_id">
            ...
            <div class="modal-body">
                ...
                <div id="editPurposeContent" style="display: none;">
                    <form id="editPurposeForm">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Discount Value</label>
                            <input type="number" class="form-control" id="edit_discount_fee" name="discount_fee" step="0.01" min="0">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Route To Administrator</label>
                            <select class="form-select" id="edit_routed_admin_id" name="routed_admin_id">
                                <option value="">Select administrator</option>
                            </select>
                        </div>
                    </form>
...
async function loadPurposes() {
    ...
    try {
        // Load both purposes and admins in parallel
        const [purposesRes, adminsRes] = await Promise.all([
        ...
        if (loadingEl) loadingEl.innerHTML = '<div class="alert alert-danger">Failed to load purposes</div>';
    }
}

function populateAdminDropdowns() {
    const addSelect = document.querySelector('#addPurposeForm select[name="routed_admin_id"]');
    const editSelect = document.getElementById('edit_routed_admin_id');
    ...
    if (addSelect) {
        addSelect.innerHTML = '<option value="">Select administrator</option>' + options;
    }
            ...
            <div class="col-md-6 col-lg-4 mb-3">
                <div class="card purpose-card h-100 border shadow-sm">
                    ...
                    <div class="card-footer bg-white border-top d-flex justify-content-end gap-2">
                        <button class="btn btn-sm btn-outline-primary" onclick="editPurpose(${purpose.purpose_id})">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <button class="btn btn-sm btn-outline-danger" onclick="deletePurpose(${purpose.purpose_id})">
...
if (addPurposeForm) {
    addPurposeForm.addEventListener('submit', async function(e) {
        ...
        const data = {
            ...
            routed_admin_id: parseInt(formData.get('routed_admin_id')) || null
        };
...
window.editPurpose = async function(purposeId) {
    ...
    try {
        ...
        const adminSelect = document.getElementById('edit_routed_admin_id');
        if (adminSelect && purpose.routed_admin_id) {
            adminSelect.value = purpose.routed_admin_id;
        }
...
if (savePurposeBtn) {
    savePurposeBtn.addEventListener('click', async function() {
        ...
        const formData = {
            ...
            routed_admin_id: parseInt(document.getElementById('edit_routed_admin_id')?.value) || null
        };
